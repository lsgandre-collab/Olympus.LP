// ============================================================
// POST /api/ares/history/offers
// Returns offer count and rating/review history
// ============================================================

import { fetchKeepaProduct } from '@/lib/ares/keepa-client'
import { processOffersHistory } from '@/lib/ares/processors'
...
import type { AresOffersHistoryData } from '@/lib/ares/types'
...
export async function POST(request: Request) {
  ...
  const days = validateDays(body.days, 365)

  const cacheKey = buildCacheKey('history_offers', { asin, domain, days })
  const cached = getCached<AresOffersHistoryData>(cacheKey)
  if (cached) {
  ...
  const data = processOffersHistory(product, days)
